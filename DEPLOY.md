...
| Daemon | npm | $0 | `npm publish @flotilla/daemon` so `npx` works. |
...
2. **Publish the daemon** so beta users can pair their computers:
   ```bash
   npm version patch && npm publish --workspace @flotilla/daemon
   ```
   Then a user runs:
   ```bash
...
4. **Landing site** (optional): deploy `apps/landing` to Netlify as a static site
   (build `npm run build --workspace @flotilla/landing`, publish `apps/landing/dist`).
...
- **Locally, boot via the workspace script** (`npm start --workspace @flotilla/api` or
